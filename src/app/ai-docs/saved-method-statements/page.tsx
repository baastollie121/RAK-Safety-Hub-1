...
const ReportContent = ({ report, onDelete }: { report: SavedStatement, onDelete: (id: string, title: string) => void }) => {
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            ...
            pdf.save(`Method-Statement-${report.title.replace(/\s+/g, '_')}.pdf`);
    ...
    return (
        <>
            <div ref={reportRef} className="prose prose-sm dark:prose-invert max-w-none rounded-lg border bg-background p-6">
                <ReactMarkdown>{report.methodStatementDocument}</ReactMarkdown>
            </div>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        delete the Method Statement for &quot;{report.title}&quot;.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                ...
                <AccordionTrigger className="p-6 hover:no-underline">
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-sm text-muted-foreground">
                      For: {report.companyName} - {report.projectTitle} | Saved on: {report.createdAt}
                    </p>
                  </div>
                </AccordionTrigger>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="font-headline flex items-center gap-2">
              <FileArchive /> No Saved Method Statements
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground">
              You haven&apos;t saved any documents yet. Go to the{' '}
              <Link href="/method-statement" className="text-primary underline">
